...
async function assertPublicHostname(hostname) {
  ...
  if (lower === 'localhost' || lower.endsWith('.local') || lower.endsWith('.internal')) {
    throw new Error('Refusing to fetch internal/local hostname');
  }
  let addresses;
  try {
  ...
  for (const { address } of addresses) {
    if (isPrivateOrLoopbackIp(address)) {
      throw new Error(`Refusing to fetch private/internal address for ${hostname}`);
    }
  }
}
...
async function safeFetch(urlString, options) {
  ...
  return fetch(urlString, { ...options, redirect: 'manual' });
}
...
Deno.serve(async (req) => {
  ...
  if (!business_id) return Response.json({ error: 'No business_id provided' }, { status: 400 });

  const biz = await sr.entities.Business.filter({ id: business_id });
  if (!biz[0]) return Response.json({ error: 'Business not found' }, { status: 404 });
  const isAdmin = user.role === 'admin';
  const ownsBusiness = biz[0].created_by_id === user.id;
  if (!isAdmin && !ownsBusiness) return Response.json({ error: 'Forbidden' }, { status: 403 });

  try {
    const parsedInput = new URL(website_url);
    if (parsedInput.protocol !== 'http:' && parsedInput.protocol !== 'https:') {
      return Response.json({ error: 'Invalid website_url protocol' }, { status: 400 });
    }
    await assertPublicHostname(parsedInput.hostname);
  } catch (e) {
    return Response.json({ error: `Invalid or disallowed website_url: ${e.message}` }, { status: 400 });
  }

  const startTime = Date.now();

  // Create audit record
  const audit = await sr.entities.SeoAudit.create({
    business_id,
    website_url,
    status: 'running',
    pages_crawled: 0,
  ...
  for (const path of [website_url, ...commonPages.map(p => baseUrl + p)]) {
    ...
    try {
      ...
      pagesCrawled++; // count any HTTP response as a crawled page
      if (res.ok) {
        html = await res.text();
        pageOk = true;
      }
  ...
  // --- Check sitemap ---
  try {
    const sitemapRes = await safeFetch(`${baseUrl}/sitemap.xml`, { signal: AbortSignal.timeout(5000) });
    if (!sitemapRes.ok) {
  ...
  for (const issue of issues) {
    await sr.entities.SeoIssue.create(issue);
  }
  ...
  // Update audit record
  await sr.entities.SeoAudit.update(audit.id, {
    status: 'completed',
