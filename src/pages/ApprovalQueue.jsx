...
export default function ApprovalQueue() {
  ...
  const { data: adDrafts = [] } = useQuery({
    queryKey: ['ads-approval'],
    queryFn: () => base44.entities.AdDraft.filter({ status: 'needs_approval' }),
  });
              ...
              <div key={lead.id} className="p-3 rounded-lg bg-muted/40 border border-border/50">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-medium">{lead.service_needed}</p>
                    <p className="text-[10px] text-muted-foreground">{lead.suburb} via {lead.source?.replace(/_/g, ' ')} — Score: {lead.score}/100</p>
                    {lead.response_draft && <p className="text-[10px] bg-card p-2 rounded mt-2 whitespace-pre-wrap">{lead.response_draft}</p>}
                  </div>
                  <div className="flex gap-1.5 flex-shrink-0">
                    <Button size="sm" variant="outline" className="h-7 text-[10px] gap-1 text-emerald-600" onClick={() => updateLeadMutation.mutate({ id: lead.id, data: { status: 'applied_responded', response_approved: true } })}><Check className="w-3 h-3" /> Approve</Button>
              ...
              <div key={ad.id} className="p-3 rounded-lg bg-muted/40 border border-border/50">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {ad.headline_options?.[0] && <p className="text-xs mt-1">{ad.headline_options[0]}</p>}
                    {ad.short_description && <p className="text-[10px] text-muted-foreground">{ad.short_description}</p>}
                  </div>
                  <div className="flex gap-1.5 flex-shrink-0">
                    ...
                    <Button size="sm" variant="outline" className="h-7 text-[10px] gap-1 text-red-500" onClick={() => updateAdMutation.mutate({ id: ad.id, data: { status: 'failed' } })}><X className="w-3 h-3" /> Reject</Button>
                  </div>
                </div>
              </div>
            ))}
