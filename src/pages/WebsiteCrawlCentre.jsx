...
export default function WebsiteCrawlCentre() {
  ...
  const crawlMutation = useMutation({
    mutationFn: ({ business_id, website_url }) => {
      ...
      return base44.functions.invoke('runSeoAudit', { business_id, website_url });
    },
            ...
            <Card key={biz.id}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" className="gap-1.5 text-xs" disabled={isCrawling || !biz.website_url}
              ...
              <CardContent>
                {!biz.website_url && (
                  <p className="text-xs text-amber-700 bg-amber-500/5 border border-amber-500/30 rounded-lg p-2 mb-3">Add and save a Website URL in Business Settings first.</p>
                )}
                {latest ? (
