...
export default function SeoControlCentre() {
  ...
  const auditMutation = useMutation({
    mutationFn: () => {
      if (!bid || !activeBusiness?.website_url) {
        throw new Error('Add and save a Website URL in Business Settings first.');
      }
      return base44.functions.invoke('runSeoAudit', { business_id: bid, website_url: activeBusiness.website_url });
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button onClick={() => auditMutation.mutate()} disabled={auditMutation.isPending || !activeBusiness?.website_url} className="gap-2">
      ...
      {!activeBusiness?.website_url && (
        <Card className="border-amber-500/30 bg-amber-500/5"><CardContent className="p-3"><p className="text-xs text-amber-700">Add and save a Website URL in Business Settings first.</p></CardContent></Card>
      )}
