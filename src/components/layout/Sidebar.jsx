...
import {
  ...
  CalendarDays, Users, FileText, Receipt, Calculator, LogOut, ScanLine,
  UserCog, Wallet, Target, ClipboardList
} from 'lucide-react';
...
const navItems = [
  ...
  { label: 'Bookings & Schedule', icon: CalendarDays, path: '/bookings' },
  { label: 'Clients', icon: Users, path: '/clients' },
  { label: 'Quote Calculator', icon: Calculator, path: '/calculator' },
  { label: 'Quotes', icon: FileText, path: '/quotes' },
  { label: 'Invoices', icon: Receipt, path: '/invoices' },
  { label: 'Scope of Work', icon: ClipboardList, path: '/scope-of-work' },
  { label: 'Screenshot Quote Generator', icon: ScanLine, path: '/screenshot-quotes' },
  { label: 'Staff Roster', icon: UserCog, path: '/staff' },
  { label: 'Expenses', icon: Wallet, path: '/expenses' },
  { label: 'Competitor Watchlist', icon: Target, path: '/competitors' },
...
export default function Sidebar({ activeBusiness, onBusinessChange, businesses, unreadCount, onClose, currentUser }) {
        ...
        {currentUser && (
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[10px] font-medium truncate">{currentUser.full_name || currentUser.email}</p>
              <p className="text-[9px] text-muted-foreground capitalize">{currentUser.role || 'user'}</p>
            </div>
            <button onClick={handleLogout} className="p-1.5 rounded hover:bg-muted flex-shrink-0" title="Logout">
